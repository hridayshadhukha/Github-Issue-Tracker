...
const displayCards = (cards) => {
  ...
  cards.forEach((card) => {
    ...
    allCards.innerHTML = `
      <div class="bg-white shadow-md border-t-4 border-[#00A96E] rounded-lg">
        <div class="flex flex-col gap-3 px-3 py-6">
          
          <div class="flex justify-between items-center">
            <div>${statusImage}</div>

            <div class="badge px-5 py-3 text-base rounded-full uppercase ${badgeColor}">
              ${card.priority}
            </div>
          </div>

          <h2 class="text-base text-[#1F2937] font-semibold line-clamp-1">
            ${card.title}
          </h2>
